...
int main()
{
	...
	for (d = alldevs; d != NULL; d = d->next)
	{
		...
		{
			cout << d->description << endl;	//输出描述信息
		}
		else
		{
			cout << "无描述信息" << endl;
	...
	strcpy(ip, inet_ntoa(((struct sockaddr_in*)(d->addresses)->addr)->sin_addr));//获取该网络接口设备的ip地址信息
	pcap_t* adhandle;//网络接口设备的句柄
	...
	else
	{
		...
		pcap_freealldevs(alldevs);//释放获取的设备列表
	}
	//设置ARP帧的内容
	...
	for (int i = 0; i < 6; i++)
	{
		ARPFrame.SendHa[i] = IPPacket->SendHa[i];//将ARPFrame.SendHa设置为本机网卡的MAC地址
		ARPFrame.FrameHeader.SrcMAC[i] = IPPacket->SendHa[i];//将ARPFrame.FrameHeader.SrcMAC设置为本机网卡的MAC地址
	}
	cout << "请输入请求的目的IP：" << endl;
	cin >> ip1;
	ARPFrame.RecvIP = inet_addr(ip1);	// 将ARPFrame.RecvIP设置为请求的IP地址
	if (pcap_sendpacket(adhandle, (u_char*)&ARPFrame, sizeof(ARPFrame_t)) != 0)
	{
		cout << "发送失败，退出程序" << endl;
		return -1;
	...
	while (1)//可能会有多条消息
	{
		...
		int rtnNew = pcap_next_ex(adhandle, &pkt_header1, &pkt_data1);	// 捕获数据包
		// ARPFrame.RecvIP = inet_addr(ip1);	// 将ARPFrame.RecvIP设置为请求的IP地址
		// if (pcap_sendpacket(adhandle, (u_char*)&ARPFrame, sizeof(ARPFrame_t)) != 0)
		// {
		// 	cout << "发送失败，退出程序" << endl;
		...
		if (rtnNew == 1)
		{
			IPPacket1 = (ARPFrame_t*)pkt_data1;	// 将捕获的数据包转换为IPPacket结构体
